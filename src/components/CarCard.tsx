    ...
    <Link href={`/cars/${id}`}>
      ...
      >
        ...
        <Stack px={2} spacing={1}>
          <CustomChip icon="heroicons:calendar-solid" text={String(year)} />
          ...
          <Stack direction={"row"} spacing={1} alignItems={"baseline"} py={2}>
            <Typography
              variant="body1"
              fontSize={"small"}
              fontWeight={700}
...
function CustomChip({ icon, text }: { icon?: string; text: string }) {
  return (
    ...
    >
      {icon && <Iconify icon={icon} size={18} sx={{ color: "grey.600" }} />}
